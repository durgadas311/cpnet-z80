; Disassembled May 20, 2020 from CP/NET 1.2 CCP.SPR
; Based on OS2CCP.ASM from CP/M 2.2.
...
;	California, 93950
;
LSTDRT	equ	12	; offset of LSTDRT flag in NDOS
...
combuf:
	db	'        '	;8 character fill
	db	'        '	;8 character fill
	db	'COPYRIGHT (C) 1980-82, DIGITAL RESEARCH '; 40
...
rrandf	equ	33	;read random record
wrandf	equ	34	;write random record
fsizef	equ	35	;compute file size
dfreef	equ	39	;free drive(s)
errmod	equ	45	;set BDOS error mode
;
netcfg	equ	69	;get network config table
cmpattr	equ	70	;set compatability attributes
;
;	special fcb flags
...
openc:	;open comfcb
	...
	lxi h,comfcb+6! mov a,m! ori 080h! mov m,a ; set F6
...
;
delete:	;delete the file given by d,e. works on CP/M 1.4
	push d! mvi c,getver! call bdos! pop d ;OS ver in HL
	mov a,l! cpi 020h ;test if pre-2.0
	mvi c,delf! jnc bdos ;2.0 or later, normal path
	call bdos! xra a! ret ;1.4, force success
;
bdos$cond:
	call bdos! ora a! ret
;
...
;
randread:
	;read random record from the file given by d,e
	mvi c,rrandf! jmp bdos$cond
;
randwrite:
	;write random record from the file given by d,e
	mvi c,wrandf! jmp bdos$cond
;
filesize:
	;compute file size for the file given by d,e
	mvi c,fsizef! jmp bdos$cond
;
...
readcom:
	...
	lda submit! ora a! jz nosub
		...
		; ------------- CP/NET ---------------
		; No more dirty tricks with SUB files.
		; CP/NET servers (MP/M) won't tolerate that.
		; Maintain current record number in last record
		; of $$$.SUB, use that to access the current command.
		; $$$.SUB does not shrink during processing, but
		...
		lxi d,subfcb! call filesize ;get last rec num
		lhld subrr! mov a,l! ora h! jz nosub ;empty file
		dcx h! shld subrr! shld lastrr ; size-1 is last rec
		lxi d,subfcb! call randread; read last record
		jnz nosub
			lda buff! cpi 0ffh! jnz gotrec ;no flag, rec has cmd
			lhld buff+1! shld subrr ;set cur cmd rec
			lxi d,subfcb! call randread! jnz nosub
			gotrec:
			;disk read is ok, transfer to combuf
			lxi d,comlen! lxi h,buff! mvi b,128! call move0
			;line is transferred, close the file with a
			;deleted record
			lhld subrr! mov a,h! ora l! jnz sub0 ;more commands
				;no more commands, delete sub file
				call del$sub! jmp sub1 ;done, delete file
			sub0: dcx h! shld buff+1 ;one less record
				mvi a,0ffh! sta buff ;flag indir recnum
				lhld lastrr! shld subrr
				lxi d,subfcb! call randwrite ;update file
			sub1:	jmp sub2
			sub2:
	...
	adi 'A'! call printchar
	mvi a,'>'! call printchar
readcomx: ;entry for ERA command - force user input
	...
	mov a,b! ora a! jnz readcom0 ;check empty buf
		;null command, bail early
		pop h! jmp ccp1
...
ccpstart:
	...
	mov a,m! ral! jnc notnet ; drive A: networked?
		;drive A: on network, must manually search for $$$.SUB
		lxi d,subfcb! call search! inr a! jz setsub
			mvi a,0ffh
		setsub:	sta submit
	notnet:
...
ccp:
	...
	lxi sp,stack
	;communicate warmboot event to servers
	mvi c,dfreef! lxi d,0ffffh! call bdos ;reset net servers
ccp1:	;enter here if user inputs empty line in readcom
	lxi h,ndos! lxi d,LSTDRT! dad d ;get NDOS LSTDRT flag addr
	push h
	mov a,m! ora a! jz nolst ; check LSTDRT flag in NDOS
	inx h! mov a,m! ora a! jnz nolst ;check CTLPF flag in NDOS
		;LSTDRT and not CTLPF - need to end print job
		mvi c,netcfg! call bdos ;ntwk cfg table to HL
		lxi d,36! dad d ;LST: ntwk cfg
		mov a,m! ral! jnc nolst ; LST: networked?
			mvi e,0ffh! mvi c,lstout! call bdos ;end-job on LST:
	nolst: pop h! xra a! mov m,a ;clear LSTDRT flag
 if 0	;readcom now displays the prompt
	call crlf ;print d> prompt, where d is disk name
	call cselect ;get current disk number
	adi 'A'! call printchar
	mvi a,'>'! call printchar
...
userfunc:
	...
	lda comfcb+1! cpi ' '! jnz user0
		...
		dcr a
		push psw! call select! pop psw
		sta cdisk! call setdiska ;set user/disk
		jmp endcom
	user0:	;file name is present
		...
		;setup compat mode from COM file attrs F1'-F4'
		lxi h,comfcb+1! lxi b,00400h	;b=4, c=0
		compat0:
			;move file attr into next bit in C
			mov a,m! ral! mov a,c! ral! mov c,a
			inx h! dcr b! jnz compat0 ;next attr
		mov a,c! add a! add a! add a! add a ;attrs in bits 7-4
		mov e,a ;ready for BDOS call
		ani 0001$0000b! jz compat1 ;test F4'
			;F4' forces bits 6,5,4
		...
		bmove3:	;b has character count
			...
			lda curusr! mov e,a! call setuser
			call setdiska! lda cdisk! call select
			jmp ccp
		;
...
subrr:	ds	3	;random record number
...
; NDOS starts on next page
here	equ	$-ccploc
	ds	((here+0ffh) and 0ff00h)-here
ndos:	end	ccploc
